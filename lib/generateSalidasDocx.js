...
export async function generateAnexo4SalidaDocx(data) {
  try {
    ...
    doc.render({
      distrito: data.distrito || 'Esteban Echeverría',
      ...
      lugarSalida: data.lugarSalida || 'Sede CENS Nº 454',
      fechaRegreso: data.fechaRegreso || new Date().toLocaleDateString('es-AR'),
      horaRegreso: data.horaRegreso || '18:00 hs',
      lugarRegreso: data.lugarRegreso || 'Sede CENS Nº 454',
      obsFechas: data.obsFechas || 'Sujeto a condiciones climáticas favorables',
      itinerario: data.itinerario || 'Recorrido pedag?gico seg?n plan de estudios.',
      actividades: data.actividades || 'Actividades de campo y producción de informe.',
      ...
      lugarFecha: 'Esteban Echeverría, ' + new Date().toLocaleDateString('es-AR'),
      autoridad: 'Direcci?n CENS Nº 454'
    });
